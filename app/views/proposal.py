...
def update_proposal_view(proposal_id: int, proposal: ProposalSchema, db: Session = Depends(get_db)):
    return proposal_controller.update_proposal(db=db, proposal_id=proposal_id, proposal=proposal)

@router.delete("/{proposal_id}")
def delete_proposal_view(proposal_id: int, db: Session = Depends(get_db)):
